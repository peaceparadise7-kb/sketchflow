...
export const StarBorderButton: React.FC<StarBorderButtonProps> = ({
  ...
  return (
    <div className={`relative inline-block rounded-full p-[1px] overflow-hidden group ${className}`}>
      {/* Slow Rotating Star Border Conic Highlight */}
      <div
        className="absolute inset-[-100%] animate-[spin_7s_linear_infinite] opacity-75 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
        style={{
          background: `conic-gradient(from 0deg, transparent 0 340deg, ${color} 360deg)`,
        }}
      />
      {/* Inner Child Button Container */}
